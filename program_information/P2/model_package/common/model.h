...
typedef struct {
  uint8_T is_active_c1_model;          /* '<Root>/Vending_Machine' */
  uint8_T is_c1_model;                 /* '<Root>/Vending_Machine' */
} DW;

/* External inputs (root inport signals with default storage) */
typedef struct {
  int8_T coin;                         /* '<Root>/coin' */
} ExtU;

/* External outputs (root outports fed by signals with default storage) */
typedef struct {
  boolean_T soda;                      /* '<Root>/soda' */
} ExtY;
